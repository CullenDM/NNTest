...
static inline uint8_t ternary_to_bits(int8_t value) {
  switch (value) {
    case 1:
      return 0x1;
    case -1:
...
static inline int8_t bits_to_ternary(uint8_t bits) {
  switch (bits & 0x3) {
    case 0x1:
      return 1;
    case 0x2:
...
void TernaryLinear::set_weight_ternary(const std::vector<float> &w, float threshold) {
  ...
  for (size_t i = 0; i < w.size(); ++i) {
    const float val = w[i];
    if (val > threshold) {
      set_weight_value(i, 1);
    } else if (val < -threshold) {
      set_weight_value(i, -1);
    } else {
      set_weight_value(i, 0);
    }
  }
}

void TernaryLinear::set_weight_value(size_t idx, int8_t value) {
  if (idx >= weight_count) {
    return;
  }
  const size_t byte_index = idx / 4;
  const uint8_t shift = static_cast<uint8_t>((idx % 4) * 2);
  const uint8_t mask = static_cast<uint8_t>(0x3 << shift);
  const uint8_t bits = static_cast<uint8_t>(ternary_to_bits(value) << shift);
  weight_packed[byte_index] = static_cast<uint8_t>((weight_packed[byte_index] & ~mask) | bits);
}

int8_t TernaryLinear::get_weight_value(size_t idx) const {
  if (idx >= weight_count) {
    return 0;
  }
  const size_t byte_index = idx / 4;
  const uint8_t shift = static_cast<uint8_t>((idx % 4) * 2);
  const uint8_t bits = static_cast<uint8_t>((weight_packed[byte_index] >> shift) & 0x3);
  return bits_to_ternary(bits);
}
    ...
    for (int s = 0; s < x.S; ++s) {
      for (int o = 0; o < out_f; ++o) {
        ...
        for (int i = 0; i < in_f; ++i) {
          const int8_t w = get_weight_value(w_base + static_cast<size_t>(i));
...
void SuperAgent::forward(const std::vector<uint8_t> &byte_input, int B, int S, Tensor3 &logits, Tensor3 &x_glob, bool update_memory) {
  ...
  for (int h = 0; h < cfg.byte_embed_dim; ++h) {
    ...
    for (int i = 0; i < ctx_proj.in_f; ++i) {
      const int8_t w = ctx_proj.get_weight_value(w_base + static_cast<size_t>(i));
      if (w == 1) {
